...
/***** cette partie est dédié au traitement du Array arr  *****/

// ajouter un niveau pour être aligné à ingredient !!
flatObjet = [];
arr.forEach((o) => {
    o.ingredients.forEach((ingred) => {
        // boucle dans ingrédients 
        ...
        } else if (ingred.quantity !== undefined) {
            flatObjet.push({
                "id": o.id,
                "name": o.name,
...
//console.log("flatObjet", flatObjet);

// flatObjet est X par le nombre d'ingredients 
// recipes enlève la duplication via l'id
var recipes = [];
flatObjet.forEach(o => {  // remove duplicate
    let temp = recipes.find(x => {
        if (x && x.id === o.id) { // rassembler autour de l'id 
            x.ingredient += '<br/>' + o.ingredient;
            return true;
        }
    });
    if (!temp)
...
/***** cette partie est dédié à l'affichage à partire de la donnée arr  ****/
function render(data) {
    if (data.length > 0) {
        var recipesHTMLString = '<div class="row">' +
            data.map(function (recipes) {
                return '<div class="col-md-4 labox"> <div class="card mb-4 box-shadow">' +
                    '<svg width="100%" height="100%" viewBox="0 0 380 178" fill="none\" xmlns="http://www.w3.org/2000/svg">' +
                    '<path d="M0 5C0 2.23858 2.23858 0 5 0H375C377.761 0 380 2.23858 380 5V178H0V5Z" fill="#C7BEBE" />' +
                    '</svg> <div class="card-body"> <div class="row mb-3"> <div class="col-8"> <b>' +
                    recipes.name + '</b></div><div class="col-4"><div class="float-right"><h5><svg class="horloge" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M10 0C4.5 0 0 4.5 0 10C0 15.5 4.5 20 10 20C15.5 20 20 15.5 20 10C20 4.5 15.5 0 10 0ZM10 18C5.59 18 2 14.41 2 10C2 5.59 5.59 2 10 2C14.41 2 18 5.59 18 10C18 14.41 14.41 18 10 18ZM10.5 5H9V11L14.2 14.2L15 12.9L10.5 10.2V5Z" fill="black" /> </svg> 10 min </h5></div></div></div><div class="row liste"><div class="col top">' +
                    recipes.ingredient + '</div><div class="col"><p class="block-ellipsis">' +
                    recipes.description + '</p></div><div class="sr-only">' +
                    recipes.appliance + '</div><div class="sr-only">' +
                    recipes.ustensils + '</div></div></div></div></div>';
            }).join('');
        + '</div>';
    } else {
        var recipesHTMLString = 'Aucune recette ne correspond à votre critère… vous pouvez chercher « tarte aux pommes », « poisson », etc';
    }
    app.innerHTML = recipesHTMLString;
}
render(recipes);
...
* preparation du array et boucle : tostring, tolowercase, trim
* filteredList : faire le match entre le searchTermRegex et recipe (array)
* retour : la liste des recettes et celle des filtres
*/
...
var handleSearch = function (event) {
    ...
    var tokens = searchTerm
        ...
        .filter(function (token) {
            return token.trim() !== ''; // enleve l(espace)
        });
...
function renderAppareil(data) {
    if (data.length > 0) {
        let leresult = data.map(function (recipes) {
            return recipes.appliance;
        });
        //console.log("leresult 2", typeof leresult);
        let unique = leresult.reduce(function (a, b) {
            if (a.indexOf(b) < 0) {
                a.push(b);
            }
        ...
        var appareilHTMLString = 'Aucune ';
    }
    appAppareil.innerHTML = appareilHTMLString;
}
renderAppareil(recipes);


var appUstensiles = document.getElementById('appUstensiles');
function renderUstensiles(data) {
    if (data.length > 0) {
        let leresult = data.map(function (recipes) {
            return recipes.ustensils;
        });
        //console.log("leresult 3", leresult);
        var merged = [].concat.apply([], leresult);
        // console.log("merged 3", merged);
        let unique = merged.reduce(function (a, b) {
            if (a.indexOf(b) < 0) {
                a.push(b);
            }
            return a;
        ...
        var ustensilesHTMLString = 'Aucune ';
    }
    appUstensiles.innerHTML = ustensilesHTMLString;
}
...
document.querySelector('#searchIngredients').addEventListener('keyup', function (e) {
    ...
    for (let index = 0; index < lingredient.length; index++) { //Compare & Display
        const namel = lingredient[index].textContent.toLowerCase();
        if (namel.includes(searchQuery)) {
            lingredient[index].style.display = 'block';
            var counter = namel.length;
            menuIngredient(counter);
        } else {
            lingredient[index].style.display = 'none';
        }
    }
});
function menuIngredient(counter) {
    let elm = document.getElementById('wrapper1');
    if (counter <= 6) {
        elm.className = 'wrapper wrapper1 btn-lebleu shorter';
    } else {
        elm.className = 'wrapper wrapper1 btn-lebleu active';
    }
}

//recherche dans le dom et affichage dynamique de tags ingredients
document.querySelector('#searchAppareil').addEventListener('keyup', function (e) {
    let lappareil = document.querySelectorAll('.tagsAppareils .lappareil');
    let searchQuerya = searchAppareil.value.toLowerCase();// Get Search Query
    for (let index = 0; index < lappareil.length; index++) { //Compare & Display
        const namea = lappareil[index].textContent.toLowerCase();
...
function menuAppareil(countera) {
    let elm2 = document.getElementById('wrapper2');
    if (countera <= 6) {
        elm2.className = 'wrapper wrapper2 btn-vert shorter';
    } else {
        elm2.className = 'wrapper wrapper2 btn-vert active';
    }
}
...
function updateViewRecipes(ladata) { // affichage des recettes
    ...
    for (var j = 0; j < labox.length; j++) {
        if (!ladata || liste[j].innerText.indexOf(ladata) > -1
            && labox[j].style.display != "none") {
            labox[j].style['display'] = 'block';
        }
        else {
            labox[j].style['display'] = 'none';
        }
